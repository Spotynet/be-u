...
import {authApi, linkApi, profileApi, userApi} from "@/lib/api";
...
export default function LinksManager() {
  ...
  const inviteProfessional = async (professionalId: number) => {
    if (!placeId) {
      Alert.alert(
        "Perfil incompleto",
        "No encontramos el identificador de tu establecimiento. Vuelve a intentarlo o guarda tu perfil."
      );
      return;
    }
    try {
      await linkApi.inviteProfessionalToPlace(placeId, professionalId);
      Alert.alert("Invitación enviada", "El profesional ha sido invitado a tu establecimiento.");
      setSearch("");
      setResults([]);
      setSelectedProfessionalId("");
